		...
		<div key={movie.id} className="moviecard group">
			<Link to={`/movies/${movie.id}`} className="">
				<div className="block group overflow-hidden rounded-lg relative">
					<div className="">
						<div className="absolute top-0 left-0 h-full w-full bg-linear-to-t from-black from-20% to-transparent text-gray-300 p-4 opacity-0 flex flex-col justify-end gap-4 group-hover:opacity-100 items-end text-xs transition-all duration-300 z-10 ">
							{movie.overview && (
